...
class Emergency extends CI_Controller {
	...
	public function index()
	{
		if($this->session->has_userdata('user_name')==1){
			...
			$this->load->view('dash/emergency_v',$data);
		}
		else{
	...
	public function get_business(){
		$data  = $this->login_m->get_business_e();
		$data = json_encode($data);
		echo $data;
	}
	public function get_temp_up(){
		$business_name = $this->input->GET('business_name',TRUE);
		$data = $this->login_m->get_temp_up_e($business_name);
		$data = json_encode($data);
		echo $data;
	}
	// Create XML
	public function create_xml_em(){
		$data = $this->login_m->create_xml_em();
		$data = json_encode($data);
		echo $data;
	}
	public function get_add(){
		...
		$data = $this->login_m->get_add_em($mark_dis,$busi_name);
		$data = json_encode($data);
		echo $data;
	}
	public function do_upload(){
		...
		// Location
		$locat = $this->input->post('locat',true);
		// Capture
		...
		if ($error > 0){
		    ...
		    else
		    {
				if($size == 0){
					...
					// // End of merge image
					$data = $this->login_m->add_ad_em($cre_date,$name,$ad_con,$temp_name,$imme,$s_date,$e_date,$s_time,$e_time,$days,$over_url,$end_date_sta,$result_img,$locat); 
					$data = json_encode($data);
					echo $data;
				}
	     	}
	...
	public function update_sch(){
		...
		$data = $this->login_m->update_sch_em($tbl_id,$up_manual,$s_date,$e_date,$s_time,$e_time,$days,$end_date_sta1);
		$data = json_encode($data);
		echo $data;
	}
}