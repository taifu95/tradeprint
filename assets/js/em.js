...
$(document).ready(function(){
    ...
    $("#up_im").click(function(){
        var diff = new Date($("#end_date").val()) - new Date($("#start_date").val());
        ...
        if($("#player_id").val()==""){
            $("#error_noti_text")[0].style.fontSize = '20px';
            $("#error_noti_text")[0].textContent = "Select Emergency Broadcast Locations";
            $("#messageNotification_error").jqxNotification("open");
            return false;
        }
    ...
    var source =
    {
        ...
        dataFields: [
            ...
            { name: 'dis_type', type: 'string' },
            // get_temp_proper
            { name: 'temp_name', type: 'string' },
            { name: 'em_location', type: 'string' },
   ...
   //          if($("#ul").val() == "Primary TA" || $("#ul").val() == "Secondary TA"){
    ...
    $("#dataTable").jqxDataTable(
    {
        ...
        rendered: function () {
            ...
            if ($(".edit").length > 0) {
                $(".edit").on('click',function (event) {
                    ...
                    if(rows_det[rowIndex].dis_type== "sch"){
                        up_manual = "true";
                        $("#manual1").click();
                    ...
                    if(rows_det[rowIndex].dis_type == "imme"){
        ...
        columns: [
            ...
            { text: 'Billboard Location(s)', cellsAlign: 'left', align: 'center', dataField: 'em_location', width: "21%" ,columngroup: 'OrderDetails',
                cellsRenderer: function (row, column, value, rowData) {
                    var container = '<div style="width: 100%; height: 130px;">'
                    var leftcolumn = '<div style="float: left; width: 100%;height:55px">';
            
                    if(rowData.em_location != ""){
                ...
                cellsRenderer: function (row, column, value, rowData) {
                    ...
                    if(rowData.e_date != ""){
                        var lastname = "<div style='margin-top: 20px;border-top:1px solid #ddd;color:#00acc1'>" +
                         ...
                         var lastname = "<div style='margin-top: 20px;border-top:1px solid #ddd;color:#00acc1'>" +
                                        "<button class='btn btn-success' style='width:48%;padding:12px 12px 12px 5px;Background-color:grey'>" + rowData.e_time + "</button>"+
                                        "<button class='btn btn-success' style='width:48%;padding:12px 12px 12px 0px;Background-color:grey'>" + rowData.s_date + "</button>"+
                                        "</div>";
            ...
            { text: 'Weekdays', cellsAlign: 'left', align: 'center', dataField: 'name', width: "12%",columngroup: 'OrderDetails2',filterable:false,
                cellsRenderer: function (row, column, value, rowData) {
                    ...
                    if(w_str != ""){
                        ...
                        var lastname = "<div style='padding-top:2px;border-top:1px solid #ddd;margin-top:20px'><div style='box-shadow:0 2px 2px 0 rgba(76, 175, 80, 0.14), 0 3px 1px -2px rgba(76, 175, 80, 0.2), 0 1px 5px 0 rgba(76, 175, 80, 0.12);border-radius:0.2rem;margin-top: 4px;min-height:40px;height100%;padding-top:5px;font-size:.75rem;Background-color:grey;padding:10px 10px 10px 30px ;color:white'>Four hours</div></div>";
                    }
                    // if(w_str != ""){
                    //     var lastname = "<div style='margin-top: 20px;border-top:1px solid #ddd;color:#00acc1'>" +
